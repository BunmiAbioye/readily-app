...
<div style="max-width:560px;margin:40px auto;padding:0 20px 40px;">
  ...
  <div style="background:#fff;border-radius:16px;padding:32px 28px;border:1px solid #e8e4de;">
    ...
    ${digest.bigWin ? `
    <div style="background:#f0fdf4;border-left:3px solid #059669;border-radius:0 8px 8px 0;padding:12px 16px;margin-bottom:14px;">
      <div style="font-size:10px;font-weight:700;color:#059669;letter-spacing:0.08em;margin-bottom:4px;">🏆 BIG WIN</div>
      <div style="font-size:14px;color:#14532d;line-height:1.6;">${digest.bigWin}</div>
    </div>` : ''}

    ${digest.pattern ? `
    <div style="background:#f0f9ff;border-left:3px solid #0284c7;border-radius:0 8px 8px 0;padding:12px 16px;margin-bottom:14px;">
      <div style="font-size:10px;font-weight:700;color:#0284c7;letter-spacing:0.08em;margin-bottom:4px;">🔍 PATTERN</div>
      <div style="font-size:14px;color:#0c4a6e;line-height:1.6;">${digest.pattern}</div>
    </div>` : ''}

    ${digest.forHome?.length ? `
    <div style="background:#faf5ff;border-left:3px solid #7c3aed;border-radius:0 8px 8px 0;padding:12px 16px;margin-bottom:14px;">
      <div style="font-size:10px;font-weight:700;color:#7c3aed;letter-spacing:0.08em;margin-bottom:8px;">🏠 TRY AT HOME</div>
      ${digest.forHome.map(tip => `<div style="font-size:13px;color:#4c1d95;line-height:1.6;margin-bottom:4px;">• ${tip}</div>`).join('')}
    </div>` : ''}

    ${digest.lookAhead ? `
    <div style="background:#f7f5f2;border-radius:8px;padding:12px 16px;margin-bottom:20px;">
      <div style="font-size:10px;font-weight:700;color:#6b6760;letter-spacing:0.08em;margin-bottom:4px;">🌅 LOOKING AHEAD</div>
      <div style="font-size:13px;color:#2d2b27;line-height:1.6;">${digest.lookAhead}</div>
    </div>` : ''}

    <table width="100%" cellpadding="0" cellspacing="0">
      <tr><td align="center">
        <a href="${appUrl}" style="display:inline-block;padding:12px 28px;background:#0d9488;border-radius:10px;color:#fff;font-size:14px;font-weight:700;text-decoration:none;">
          Open Readily →
        </a>
      </td></tr>
    </table>
  ...
  <div style="text-align:center;padding:16px 0 0;">
    <p style="margin:0;font-size:11px;color:#a8a49e;line-height:1.6;">
      Readily by AblePam Inc. · <a href="${appUrl}" style="color:#0d9488;text-decoration:none;">readily.ablepam.ca</a><br>
      <a href="${appUrl}/settings" style="color:#a8a49e;text-decoration:none;">Manage digest settings</a>
    </p>
  </div>
</div>
</body>
        ...
        console.error(`[Readily] Digest error for family ${family.id}:`, e);
        errors++;
      }
    }
